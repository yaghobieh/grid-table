...
const productColumns: ColumnDefinition<Product>[] = [
  ...
  { id: 'price', accessor: 'price', header: 'Price', sortable: true, align: 'right', width: 100, render: (v) => `$${Number(v).toLocaleString()}` },
  { id: 'stock', accessor: 'stock', header: 'Stock', sortable: true, align: 'right', width: 80 },
  { id: 'rating', accessor: 'rating', header: 'Rating', sortable: true, align: 'center', width: 80, render: (v) => `${v} ★` },
  { id: 'status', accessor: 'status', header: 'Status', sortable: true, width: 120, render: (v) => <StatusDot color={STATUS_COLORS[String(v)] ?? '#64748b'} label={String(v)} /> },
];
...
const taskColumns: ColumnDefinition<Task>[] = [
  ...
  { id: 'priority', accessor: 'priority', header: 'Priority', sortable: true, width: 100, render: (v) => <Badge variant="secondary" className="text-xs" style={{ borderLeft: `3px solid ${PRIORITY_COLORS[String(v)] ?? '#888'}` }}>{String(v)}</Badge> },
  { id: 'status', accessor: 'status', header: 'Status', sortable: true, width: 120, render: (v) => <StatusDot color={TASK_STATUS_COLORS[String(v)] ?? '#64748b'} label={String(v)} /> },
  { id: 'dueDate', accessor: 'dueDate', header: 'Due Date', sortable: true, width: 110, render: (v) => new Date(String(v)).toLocaleDateString() },
          ...
          <div className="dark">
            <GridTable
              ...
              statusBar={{
                ...
                aggregations: [
                  { columnId: 'price', type: 'avg', label: 'Avg Price', format: (v) => `$${Math.round(v).toLocaleString()}` },
                  { columnId: 'stock', type: 'sum', label: 'Total Stock' },
                  { columnId: 'rating', type: 'avg', label: 'Avg Rating', format: (v) => v.toFixed(1) },
                ],
              }}
            ...
            <GridTable
              ...
              statusBar={{
                ...
                aggregations: [
                  ...
                  { columnId: 'effort', type: 'avg', label: 'Avg Effort', format: (v) => v.toFixed(1) },
                ],
              }}
            />
          </div>
          ...
          <div className="dark">
            <GridTable
              ...
              statusBar={{
                ...
                showRowCount: true,
                aggregations: [
                  { columnId: 'salary', type: 'sum', label: 'Total Payroll', format: (v) => `$${v.toLocaleString()}` },
                ],
              }}
            />
          </div>
